...
public class V3CartServiceImpl implements V3CartService {
     ...
     * 增加购物车
     *
     * @param userKey   sessionId或者userId
     * @param sid       商铺信息
     * @param num       商品数量
     * @param goodSkuId
     * @param type      0 为未登陆
     ...
     *
     * @param sellerId 店铺ID
     * @param skuId    如果商品信息为空，则是全选或或全部取消
     * @param userId   用户ID ，如果没登录则是会话ID
     */
    ...
    public void delete(CartInfo cart) {
        Criteria where = Criteria.where("userId").is("" + cart.getUserId()).and("sellerId")
                .is(cart.getSellerId()).and("skuId").is(cart.getSkuId());
        Query query = Query.query(where);
        mongo.remove(query, CartInfo.class);
    }
